...
# Now again for different xi
# Q_0
exact_0_p = read_file("analytic_0_p_2.txt")
exact_0_m = read_file("analytic_0_m_2.txt")
avg_0_p = read_file("average_0_p_2.txt")
avg_0_m = read_file("average_0_m_2.txt")
err_0_p = read_file("error_0_p_2.txt")
err_0_m = read_file("error_0_m_2.txt")
gamma_0 = read_file("gamma_0_2.txt")
mu_0_p = (np.sqrt(3)+1)/2
mu_0_m = (np.sqrt(3)-1)/2
exact_0 = mu_0_p*exact_0_p - mu_0_m*exact_0_m
avg_0 = mu_0_p*avg_0_p - mu_0_m*avg_0_m
...
# Q_x
exact_x_p = read_file("analytic_x_p_2.txt")
exact_x_m = read_file("analytic_x_m_2.txt")
avg_x_p = read_file("average_x_p_2.txt")
avg_x_m = read_file("average_x_m_2.txt")
err_x_p = read_file("error_x_p_2.txt")
err_x_m = read_file("error_x_m_2.txt")
gamma_x = read_file("gamma_x_2.txt")
exact_x = .5*(exact_x_p - exact_x_m)
avg_x = .5*(avg_x_p - avg_x_m)
err_x = .5*np.sqrt(err_x_p**2 + err_x_m**2)

# Q_y
exact_y_p = read_file("analytic_y_p_2.txt")
exact_y_m = read_file("analytic_y_m_2.txt")
avg_y_p = read_file("average_y_p_2.txt")
avg_y_m = read_file("average_y_m_2.txt")
err_y_p = read_file("error_y_p_2.txt")
err_y_m = read_file("error_y_m_2.txt")
gamma_y = read_file("gamma_y_2.txt")
exact_y = .5*(exact_y_p - exact_y_m)
avg_y = .5*(avg_y_p - avg_y_m)
err_y = .5*np.sqrt(err_y_p**2 + err_y_m**2)

# Q_z
exact_z_p = read_file("analytic_z_p_2.txt")
exact_z_m = read_file("analytic_z_m_2.txt")
avg_z_p = read_file("average_z_p_2.txt")
avg_z_m = read_file("average_z_m_2.txt")
err_z_p = read_file("error_z_p_2.txt")
err_z_m = read_file("error_z_m_2.txt")
gamma_z = read_file("gamma_z_2.txt")
exact_z = .5*(exact_z_p - exact_z_m)
avg_z = .5*(avg_z_p - avg_z_m)
err_z = .5*np.sqrt(err_z_p**2 + err_z_m**2)
